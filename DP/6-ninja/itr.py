class Solution:
    def ninjaTraining(self, matrix):
        ...
        maxCounter=[0,0,0]#[matrix[0][0],matrix[0][1],matrix[0][2]]
        
        for j in range(0,len(matrix)):
            # for 0 index
            temp1 = max(maxCounter[1] , maxCounter[2] )
            temp2 = max(maxCounter[0] , maxCounter[2] )
            temp3 = max(maxCounter[0] , maxCounter[1] )
            maxCounter[0] = temp1 + matrix[j][0]
            maxCounter[1] = temp2 + matrix[j][1]
            maxCounter[2] = temp3 + matrix[j][2]
            #print("After iteration", j, ":", maxCounter)
        maxVal=max(maxCounter[0],maxCounter[1],maxCounter[2])
