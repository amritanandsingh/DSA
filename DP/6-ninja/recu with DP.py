...
class Solution:
    def fun(self,matrix,index,lastIndex ,dp):
        ...
        running=0
        stealth=0
        if(dp[index] != -1 and dp[index][lastIndex] != -1):
            return dp[index][lastIndex]
        if lastIndex != 0:
            running = self.fun(matrix,index-1,0,dp) + matrix[index][0]
        if lastIndex != 1:
            stealth = self.fun(matrix,index-1,1,dp ) + matrix[index][1]
        if lastIndex != 2:
            fighting = self.fun(matrix,index-1 , 2,dp) + matrix[index][2]

        dp[index][lastIndex]=max(running,stealth,fighting)
        return dp[index][lastIndex]
    ...
    def ninjaTraining(self, matrix):
        ...
        for i in range(len(matrix)):
            dp.append([-1,-1,-1])
        ans = self.fun(matrix,len(matrix)-1,-1,dp)
        #print(dp)
        return ans
